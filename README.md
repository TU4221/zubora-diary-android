...
> **⚠️ 既知の不具合 (Known Issues)**
>
> * **現象:** アプリ操作時（画面遷移が伴うボタン押下時）に、稀にUIがフリーズする現象を確認しています。
> * **原因:** 内部で使用しているライブラリの遅延初期化処理（`by lazy`）において、バックグラウンドスレッドとメインスレッドの競合（Long monitor contention）が発生しているためと特定しています。
> * **対応:** 現在、詳細な発生箇所の特定と修正作業を進めています。次回のアップデートで修正予定です。
