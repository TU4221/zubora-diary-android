...
public class DiaryListSimpleCallback extends ItemTouchHelper.SimpleCallback {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (event.getAction() == MotionEvent.ACTION_UP) {
                    ...
                    if (lastMotionEvent == MotionEvent.ACTION_DOWN) {
                        result = processOnTouchingSwipedViewHolder(v, event);
                    }
                }
    ...
    private void closeSwipedViewHolder(int position) {
        if (position < 0) throw new IllegalArgumentException();

        Log.d("LeftSwipingCallBack_", "closeSwipedViewHolder()_position:" + position);
        RecyclerView.ViewHolder viewHolder =
    ...
    private void animateSwipingView(
    ...
    public float getSwipeThreshold(@NonNull RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder instanceof LeftSwipeViewHolder) {
            ...
            // TODO:この処理必要？
            if (swipedAdapterPosition != viewHolder.getBindingAdapterPosition()) return 1f;
            return 0f;
        }
        ...
        if (swipedAdapterPosition != viewHolder.getBindingAdapterPosition()) {
            Log.d("LeftSwipingCallBack_", "getSwipeThreshold()_return:" + threshold);
            return threshold;
        }

        Log.d("LeftSwipingCallBack_", "getSwipeThreshold()_return:" + (1 - threshold));
        return 1 - threshold;
    }
